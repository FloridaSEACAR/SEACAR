...
colnames(DO)[colnames(DO) == "X.DissolvedOxygenSaturation_.."] <- "DO"
...
boxplot(DO ~ yearmonth, data = DO3surf, xlab = "yearmonth",
        ylab = "DO, saturation", main = "DO boxplot", notch=FALSE)
#then at year level
...
plots_listyrmth <- lapply(unique(DO3surf$ManagedAreaName), function (i) {
  ...
  ggplot(data = dat, aes(x = yearmonth, y = DO)) +
    ...
    labs(title = i, y="DO (saturation)") 
})
#make a pdf with all of these
pdf("Quality_DOsat_MA_surf_MonthlyBoxplots.pdf", width = 16, height = 4)
...
#make a pdf with all of these
pdf("Quality_DOsat_MA_surf_AnnualBoxplots.pdf", width = 16, height = 4)
print(plots_listyr)
dev.off()
...
##
pdf("2Quality_DOsat_MA_surf_plots.pdf", width = 16, height = 4)
print(plots_list)
dev.off()
...
sink("Quality_DOsat_MA_surf_KToutput.csv")
...
write_csv(kendall_with_regions_final_surface, "Quality_DOsat_MA_surf_resultstables.csv")
...
colnames(SUMMARYDOsurfmeanmedstdev) [4] <- "Minimum Value (DO, saturation)" 
colnames(SUMMARYDOsurfmeanmedstdev) [5] <- "Maximum Value (DO, saturation)" 
colnames(SUMMARYDOsurfmeanmedstdev) [6] <- "Median (DO, saturation)" 
colnames(SUMMARYDOsurfmeanmedstdev) [7] <- "Mean (DO, saturation)" 
colnames(SUMMARYDOsurfmeanmedstdev) [8] <- "Standard Deviation (DO, saturation)" 
colnames(SUMMARYDOsurfmeanmedstdev) [9] <- "Programs with Data" 

write.csv(SUMMARYDOsurfmeanmedstdev,"Quality_DOsat_MA_surf_resultssummarystats2.csv")
