#FLORIDA SEACAR
#SAV Scripts, % cover by turbidity
...
#This script uses mixed effects models to find the effect of Turbidity on Percent Cover 
...
turbid<-read.csv("~/Desktop/turbidforlm.csv")
colnames(turbid)

df <- inner_join(SAV, turbid, by = c("Region", "ManagedAreaName","Year","Month","yearmonth"))  # Applying inner_join() function
df       
colnames(df)
colnames(df) [7]<-"PercentCover"
colnames(df) [9]<-"turbidity"
...
model<-lapply(unique(df$ManagedAreaName), 
                         function(i)summary(lme(PercentCover ~ turbidity,
                                                random =~1|Year,
...
preds <- do.call(rbind, lapply(names(dfmodel), function(i) {
  mod <- dfmodel[[i]]
  pred <- data.frame(ManagedAreaName = i, turbidity = mod$data$turbidity, pred = predict(mod, level = 0))
}))
...
Plot_TurbidityandSAV<-ggplot() +
  geom_jitter(data = df, aes(x = turbidity, y = PercentCover, col = ManagedAreaName),size=0.1) +
  geom_line(data = preds, aes(x = turbidity, y = pred, group = ManagedAreaName, col = ManagedAreaName))
#label the plot and y axis (x was already labeled above)
Plot_TurbidityandSAV<-Plot_TurbidityandSAV + labs(y = "Percent Cover")
Plot_TurbidityandSAV<-Plot_TurbidityandSAV + labs(title = "Percent Cover x Turbidity by Managed Area")
#color code your points and lines by taxon
Plot_TurbidityandSAV<-Plot_TurbidityandSAV + labs(colour = "ManagedAreaName")
Plot_TurbidityandSAV
#print to a pdf
pdf("TurbidityandSAVmodelPlot.pdf", width = 16, height = 4)
print(Plot_TurbidityandSAV)
dev.off()
##
##
##
...
dfmodelno_summary<-lapply(unique(df$ManagedAreaName), 
                          function(i)(lme(PercentCover~ turbidity,
                                          random =~1|Year,
                                          na.action = na.omit, data = subset(df, ManagedAreaName== i)) ))
...
df_lmelist2summary <-  dfmodel%>% 
  ...
  rename(`Effect of turbidity` = Value) %>% 
  rename(df = DF) %>% 
  rename(Type = rowname)
...
joined_output_table <- left_join(df_lmelist_tidy, df_lmelist2summary, by = "ManagedArea") %>% 
  ...
  # rename(`Random Effects turbidity (SD)` = sd_turbidity) %>% ****
  rename(`Random Effects Residual (SD)` = sd_Observation) %>% 
  # rename(`Random Effects turbidity (corr)` = `cor_turbidity.(Intercept)`) %>% ****
  rename(FE_year = turbidity) %>% 
  rename(FE_intercept = `(Intercept)`) %>% 
  rename(`Fixed Effects Intercept` = FE_intercept) %>% 
  relocate(c(`Random Effects Intercept (SD)`,
             # `Random Effects turbidity (SD)`, ****
             `Random Effects Residual (SD)`
             # `Random Effects turbidity (corr)` ****
  ),
  .after = `p-value`) %>% 
  relocate(c(`Log-restricted-likelihood`,
             `Number of Observations`,
             `Number of Groups`),
           .after = `Random Effects Residual (SD)`) %>% #**** `Random Effects turbidity (corr)` to `Random Effects Residual (SD)`
  select(-FE_year) %>% 
...
write.csv(joined_output_table,"TurbidityandSAVmodelTable.pdf.csv")
