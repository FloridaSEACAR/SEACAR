...
SAV4_sum <- SAV4 %>% group_by(ManagedAreaName) %>% summarize(n_yr = length(unique(Year)), yrs = list(sort(unique(Year))))
...
#Create a function to add multiple lme model fits to a prepared base plot (used in the process starting below)
#Case_when evaluates all rhs options before returning the value that satisfies the lhs conditions, so this version of addfits was causing errors due to missing models.
# addfits <- function(models, plot_i){
#   case_when(length(models) == 1 ~
#             return(plot_i + geom_line(data = subset(SAV4, SAV4$ManagedAreaName == i & !is.na(SAV4$BB_pct) & SAV4$analysisunit == unique(eval(models[[1]])$data$analysisunit)), 
#                                       aes(x = relyear, y = predict(eval(models[[1]]), level = 0), color = unique(eval(models[[1]])$data$analysisunit)), lwd = 1.2)),
#             length(models) == 2 ~
#             return(plot_i + geom_line(data = subset(SAV4, SAV4$ManagedAreaName == i & !is.na(SAV4$BB_pct) & SAV4$analysisunit == unique(eval(models[[1]])$data$analysisunit)), 
#                                       aes(x = relyear, y = predict(eval(models[[1]]), level = 0), color = unique(eval(models[[1]])$data$analysisunit)), lwd = 1.2) +
#                      geom_line(data = subset(SAV4, SAV4$ManagedAreaName == i & !is.na(SAV4$BB_pct) & SAV4$analysisunit == unique(eval(models[[2]])$data$analysisunit)), 
...
#                      geom_line(data = subset(SAV4, SAV4$ManagedAreaName == i & !is.na(SAV4$BB_pct) & SAV4$analysisunit == unique(eval(models[[2]])$data$analysisunit)), 
#                                aes(x = relyear, y = predict(eval(models[[2]]), level = 0), color = unique(eval(models[[2]])$data$analysisunit)), lwd = 1.2) +
#                      geom_line(data = subset(SAV4, SAV4$ManagedAreaName == i & !is.na(SAV4$BB_pct) & SAV4$analysisunit == unique(eval(models[[3]])$data$analysisunit)), 
...
#                      geom_line(data = subset(SAV4, SAV4$ManagedAreaName == i & !is.na(SAV4$BB_pct) & SAV4$analysisunit == unique(eval(models[[3]])$data$analysisunit)), 
#                                aes(x = relyear, y = predict(eval(models[[3]]), level = 0), color = unique(eval(models[[3]])$data$analysisunit)), lwd = 1.2) +
#                      geom_line(data = subset(SAV4, SAV4$ManagedAreaName == i & !is.na(SAV4$BB_pct) & SAV4$analysisunit == unique(eval(models[[4]])$data$analysisunit)), 
...
#                      geom_line(data = subset(SAV4, SAV4$ManagedAreaName == i & !is.na(SAV4$BB_pct) & SAV4$analysisunit == unique(eval(models[[4]])$data$analysisunit)), 
#                                aes(x = relyear, y = predict(eval(models[[4]]), level = 0), color = unique(eval(models[[4]])$data$analysisunit)), lwd = 1.2) +
#                      geom_line(data = subset(SAV4, SAV4$ManagedAreaName == i & !is.na(SAV4$BB_pct) & SAV4$analysisunit == unique(eval(models[[5]])$data$analysisunit)), 
...
#                      geom_line(data = subset(SAV4, SAV4$ManagedAreaName == i & !is.na(SAV4$BB_pct) & SAV4$analysisunit == unique(eval(models[[5]])$data$analysisunit)), 
#                                aes(x = relyear, y = predict(eval(models[[5]]), level = 0), color = unique(eval(models[[5]])$data$analysisunit)), lwd = 1.2) +
#                      geom_line(data = subset(SAV4, SAV4$ManagedAreaName == i & !is.na(SAV4$BB_pct) & SAV4$analysisunit == unique(eval(models[[6]])$data$analysisunit)), 
#                                aes(x = relyear, y = predict(eval(models[[6]]), level = 0), color = unique(eval(models[[6]])$data$analysisunit)), lwd = 1.2)))
# }
# 
# addfits <- function(models, plot_i){
#   fitplot <- case_when(length(models) == 1 ~
#               plot_i + geom_line(data = subset(SAV4, SAV4$ManagedAreaName == i & !is.na(SAV4$BB_pct) & SAV4$analysisunit == unique(eval(models[[1]])$data$analysisunit)), 
#                                         aes(x = relyear, y = predict(eval(models[[1]]), level = 0), color = unique(eval(models[[1]])$data$analysisunit)), lwd = 1.2),
#             length(models) == 2 ~
#               plot_i + geom_line(data = subset(SAV4, SAV4$ManagedAreaName == i & !is.na(SAV4$BB_pct) & SAV4$analysisunit == unique(eval(models[[1]])$data$analysisunit)), 
#                                         aes(x = relyear, y = predict(eval(models[[1]]), level = 0), color = unique(eval(models[[1]])$data$analysisunit)), lwd = 1.2) +
...
# spcols <- c("Thalassia testudinum" = "turquoise", 
#             "Syringodium filiforme" = "tomato",
#             "Halodule wrightii" = "springgreen4",
#             "Ruppia maritima" = "orchid",
#             "Halophila sp." = "goldenrod",
#             "Total seagrass" = "cornflowerblue",
#             "Attached algae" = "chocolate1",
#             "Drift algae" = "deeppink")
...
for(p in parameters$column){
  ...
  for(i in ma_include){
    ...
    xlab <- attributes(base$layers[[2]]$data$geometry)$bbox$xmax[[1]]
    yshift <- (attributes(base$layers[[2]]$data$geometry)$bbox$ymax[[1]] - attributes(base$layers[[2]]$data$geometry)$bbox$ymin[[1]])*0.65
    base <- base + annotate("text", x = xlab, y = ystart, label = paste0(startyear), hjust = "left")
    ...
    for(y in sort(unique(SAV4[ManagedAreaName == i & !is.na(eval(p)) & Year != startyear, Year]))){
      base <- base +
        geom_sf(data = rotate_sf(rcp_i, y_add = yadd + yshift, coast = corners[ManagedAreaName == i, Coast[1]]), 
                color = "grey50", fill = "mediumaquamarine", alpha = 0.85, lwd = 1, inherit.aes = FALSE) +
        annotate("text", x = xlab, y = ystart + yshift, label = y, hjust = "left")
      ...
      if(length(subset(locs_pts_rcp_i, locs_pts_rcp_i$LocationID %in% unique(SAV4[ManagedAreaName == i & !is.na(eval(p)) & Year == y, LocationID]))$LocationID) > 0){
        base <- base +
          geom_sf(data = rotate_sf(subset(locs_pts_rcp_i, locs_pts_rcp_i$LocationID %in% unique(SAV4[ManagedAreaName == i & !is.na(eval(p)) & Year == y, LocationID])),
                                   y_add = yadd + yshift, coast = corners[ManagedAreaName == i, Coast[1]]), aes(fill = droplevels(as.factor(ProgramID))), shape = 21, color = "black")
      }
      
      yadd <- yadd + yshift
      startyear <- startyear + 1
      ystart <- ystart + yshift
    }        
