...
library(dplyr)
#read in data
CorSE<-read.csv("~/Downloads/Percent Cover  - FLA SE-2021-Jul-26.csv")
colnames(CorSE)
...
write.csv(summarySE,"CoralSummaryStatisticsTable_Genus_SERegion.csv")
...
boxplotsCorSE<- lapply(unique(CorSE$GenusName), function (i) {
  dat <- filter(CorSEboxplots, GenusName == i)
  ggplot(data = dat, aes(group=Year, x = Year, y = Perccov)) +
    geom_boxplot() +
    labs(title = "Percent cover coral by genus, Southeast Region",subtitle=i, y="Percent Cover", x="Year") })
#view
boxplotsCorSE
#print to file
pdf("CoralBoxplots_ByGenus_SERegion.pdf", width = 16, height = 4) 
print(boxplotsCorSE)
dev.off()
...
CorSE<-anti_join(CorSE,eliminate,by=c("GenusName"))
unique(CorSE$GenusName)
colnames(CorSE)
#to run just one lme, do the following
library(nlme)
CorSE_ByGenus<-lme(Perccov ~ Year,
                   random =~1|ProgramLocationID,
                   na.action = na.omit,
                   data = CorSE)
summary(CorSE_ByGenus)
...
CorSE<-CorSE%>%filter(GenusName!="Eusmilia")
CorSE<-CorSE%>%filter(GenusName!="Meandrina")

CorSE_ByGenus_lmelist2<-lapply(unique(CorSE$GenusName), 
                               function(i)summary(lme(Perccov ~ Year,
                                                      random =~1|ProgramLocationID,
                                                      na.action = na.omit, data = subset(CorSE, GenusName == i))))

#create and add a names list to the output, with each name corresponding to one
#unique taxon for the MA/in the model
names(CorSE_ByGenus_lmelist2) <- unique(CorSE$GenusName)
CorSE_ByGenus_lmelist2

#print to a csv file
sink("Corallme_ByGenus_SERegion.csv")
CorSE_ByGenus_lmelist2
sink()

#4. Graphs
#first, convert your list of analyses to a dataframe (named preds here)
preds <- do.call(rbind, lapply(names(CorSE_ByGenus_lmelist2), function(i) {
  mod <- CorSE_ByGenus_lmelist2[[i]]
...
Plot_CorSE_ByGenus<-ggplot() +
  geom_jitter(data = CorSE, aes(x = Year, y = Perccov, col = GenusName),size=0.1) +
...
Plot_CorSE_ByGenus<-Plot_CorSE_ByGenus + labs(y = "Percent Cover")
Plot_CorSE_ByGenus<-Plot_CorSE_ByGenus + labs(title = "Percent cover coral by genus, Southeast Region")
#color code your points and lines by taxon
Plot_CorSE_ByGenus<-Plot_CorSE_ByGenus + labs(colour = "GenusName")
Plot_CorSE_ByGenus
#print to a pdf
pdf("CorallmePlot_ByGenus_SERegion.pdf", width = 16, height = 4)
print(Plot_CorSE_ByGenus)
dev.off()
...
CorSE_lmelist2no_summary<-lapply(unique(CorSE$GenusName), 
                                 function(i)(lme(Perccov ~ Year,
                                                 random =~1|ProgramLocationID,
                                                 na.action = na.omit, data = subset(CorSE, GenusName== i)) ))

names(CorSE_lmelist2no_summary) <- unique(CorSE$GenusName)
#

CorSE_lmelist2summary <-  CorSE_ByGenus_lmelist2 %>% 
  enframe() %>% 
  group_by(name) %>% 
...
joined_output_table <- left_join(CorSE_lmelist_tidy, CorSE_lmelist2summary, by = "Genus") %>% 
  select(-value) %>% 
...
joined_output_tablevec<- c("Southeast","Southeast","Southeast","Southeast",
...
write.csv(joined_output_table,"Coral_lmeResults_ByGenus_SERegion.csv")
