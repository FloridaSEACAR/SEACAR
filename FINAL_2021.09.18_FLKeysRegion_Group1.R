...
colnames(summaryKEYS) [2] <- "Coral Group" 
...
write.csv(summaryKEYS,"CoralSummaryStatisticsTable_Group1_KEYSRegion.csv")
...
boxplotsCorKEYS<- lapply(unique(CorKEYS$SpeciesGroup1), function (i) {
  dat <- filter(CorKEYSboxplots, SpeciesGroup1 == i)
  ggplot(data = dat, aes(group=Year, x = Year, y = Perccov)) +
    geom_boxplot() +
    labs(title = "Percent cover by group, FL Keys Region",subtitle=i, y="Percent Cover", x="Year") })
#view
boxplotsCorKEYS
#print to file
pdf("CoralBoxplots_ByGroup1_KEYSRegion.pdf", width = 16, height = 4) 
...
library(nlme)
#CorKEYS_ByGroup1<-lme(Perccov ~ Year,
#                     random =~1|ProgramLocationID,
#                     na.action = na.omit,
#                     data = CorKEYS)
#summary(CorKEYS_ByGroup1)
...
#
CorKEYS_ByGroup1_lmelist2<-lapply(unique(CorKEYS$SpeciesGroup1), 
                                  function(i)summary(lme(Perccov ~ Year,
                                                         random =~1|ProgramLocationID,
                                                         na.action = na.omit, data = subset(CorKEYS, SpeciesGroup1 == i))))

#create and add a names list to the output, with each name corresponding to one
#unique taxon for the MA/in the model
names(CorKEYS_ByGroup1_lmelist2) <- unique(CorKEYS$SpeciesGroup1)
CorKEYS_ByGroup1_lmelist2

#print to a csv file
sink("Corallme_ByGroup1_KEYSRegion.csv")
CorKEYS_ByGroup1_lmelist2
sink()

#4. Graphs
#first, convert your list of analyses to a dataframe (named preds here)
preds <- do.call(rbind, lapply(names(CorKEYS_ByGroup1_lmelist2), function(i) {
  mod <- CorKEYS_ByGroup1_lmelist2[[i]]
...
Plot_CorKEYS_ByGroup1<-ggplot() +
  geom_jitter(data = CorKEYS, aes(x = Year, y = Perccov, col = SpeciesGroup1),size=0.1) +
...
Plot_CorKEYS_ByGroup1<-Plot_CorKEYS_ByGroup1 + labs(y = "Percent Cover")
Plot_CorKEYS_ByGroup1<-Plot_CorKEYS_ByGroup1 + labs(title = "Percent cover by group, FL Keys Region")
#color code your points and lines by taxon
Plot_CorKEYS_ByGroup1<-Plot_CorKEYS_ByGroup1 + labs(colour = "Species Group")
Plot_CorKEYS_ByGroup1
#print to a pdf
pdf("CorallmePlot_ByGroup1_KEYSRegion.pdf", width = 16, height = 4)
print(Plot_CorKEYS_ByGroup1)
dev.off()

##
#TABLES
#non-summarized lme for broom::tidy()
CorKEYS_lmelist2no_summary<-lapply(unique(CorKEYS$SpeciesGroup1), 
                                   function(i)(lme(Perccov ~ Year,
                                                   random =~1|ProgramLocationID,
                                                   na.action = na.omit, data = subset(CorKEYS, SpeciesGroup1== i)) ))

names(CorKEYS_lmelist2no_summary) <- unique(CorKEYS$SpeciesGroup1)
#

CorKEYS_lmelist2summary <-  CorKEYS_ByGroup1_lmelist2 %>% 
  enframe() %>% 
  group_by(name) %>% 
...
joined_output_table <- left_join(CorKEYS_lmelist_tidy, CorKEYS_lmelist2summary, by = "Group1") %>% 
  select(-value) %>% 
...
joined_output_tablevec<- c("FL Keys Region","FL Keys Region","FL Keys Region","FL Keys Region")            
...
write.csv(joined_output_table,"Coral_lmeResults_ByGroup1_KEYSRegion.csv")
